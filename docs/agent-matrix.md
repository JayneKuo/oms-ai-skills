...
| `allocation` | Warehouse result, automatic/manual allocation reason, dispatch/DN/WMS/fulfillment state, remaining qty, manual allocation eligibility, dispatch release/retry, reopen-for-allocation retry, batch allocation workflows | `explain_warehouse_assignment.py`, `get_dispatch_fulfillment.py`, `get_allocation_items.py`, `check_manual_allocation.py`, `get_order_detail.py`, `get_routing_rules.py`, `manual_allocate.py`, `reopen_order.py`, `batch_allocation.py`, dispatch explain endpoint | Manual/auto/force/reopen/batch allocation belongs to allocation; every real write requires second confirmation and precheck | Assigned warehouse, dispatch number/status, DN/behind order number, WMS/warehouse processing state, SKU qty, remaining qty, dispatch explain reason, confidence; reopen replies include retry acceptance and post-retry allocation state; batch replies include per-order state | `replenishment` if shortage needs PO; `operations` only for cancel |
...
| `operations` | High-impact non-allocation cancel and cancel-specific async result interpretation | `get_order_detail.py`, `cancel_order.py`, `batch_orders.py` | Yes, only after user second confirmation; no allocation/reopen writes, no dispatch release/retry writes, and no hold-release writes | Cancel submitted/completed/ongoing result, what is confirmed, cancel-specific downstream/WMS follow-up, rejection reason, post-cancel dispatch state | Back to `allocation`/`hold` for domain-specific post-checks |
...
| Order is `WAREHOUSE_PROCESSING` / dispatched | `allocation` | Dispatch detail, DN/behind order number, WMS/warehouse processing state, dispatch explain log when reason is requested, remaining qty | `operations` for cancel only; `replenishment` if stock issue |
...
| `allocation` | Ready | Owns allocation plus dispatch/fulfillment visibility; automatic allocation reason comes from dispatch explain logs before fallback evidence. |
...
| `operations` | Ready for controlled use | Requires confirmation; cancel post-checks sales order and dispatch state only for cancel completion; release hold belongs to `hold`; reopen/dispatch retry and general fulfillment diagnosis belong to `allocation`. |
...
| Ask dispatch/DN/WMS status for a warehouse-processing order | `allocation` | Returns dispatch number, DN, warehouse, WMS/warehouse status, allocation completion, and next step. |
| Query EXCEPTION list | `exception` | Returns candidates and requires detail freshness before diagnosis. |
| Reopen non-exception order for allocation retry | `allocation` | Reports OMS rejection `Order not exception` or explains why retry is not applicable. |
