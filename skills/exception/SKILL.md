...
- Do not execute reopen, cancel, manual allocation, hold release, or replenishment creation here. Route reopen/manual allocation to `allocation`, cancel to `operations`, hold release to `hold`, and PO creation to `replenishment`.
...
- If `reserve1` or another detail field explicitly says a product is out of stock, treat that as confirmed cause and route next step to `replenishment`; reopen-for-allocation retry should happen only after inventory/replenishment is handled and the business confirms retry.
...
- Recommend the business solution: replenish inventory, fix allocation blocker, reopen-for-allocation retry after the blocker is resolved, or manual review.
...
Solution: [replenishment / allocation check / allocation reopen retry after blocker resolved / manual review]
